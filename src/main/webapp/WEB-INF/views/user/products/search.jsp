...
<head>
<title>Kết quả tìm kiếm cho ${ keySearch }</title>
<style>
.pagination {
	display: flex;
...
<body>
	<div class="well well-small">
		...
		<c:if test="${searchProducts.size() == 0 }">
			<div class="row">
				<h3>
					<span class="span6">Không có sản phẩm</span>
				</h3>
			</div>
		</c:if>
		...
		<c:if test="${searchProducts.size() > 0 }">
			...
			<div class="row-fluid">
				<ul class="thumbnails">
					<c:forEach var="item" items="${ searchProducts }" varStatus="loop">
						<li class="span4">
							<div class="thumbnail">
					...
					<c:if
							test="${ (loop.index + 1) %3 == 0 || (loop.index+1) == searchProducts.size() }">
					</c:if>			
				</ul>
			</div>
			<c:if test="${(loop.index + 1) < searchProducts.size() }">
				<div class="row-fluid" >
	 ...
	 <div class="pagination">
		<c:forEach var="item" begin="1" end="${paginateInfo.totalPage}"
			...
			<c:if test="${ (loop.index) == paginateInfo.currentPage }">
				<a
					href="<c:url value="/search/key=${ keySearch }/${ loop.index }"/>"
					class="active">${loop.index}</a>
			</c:if>

			<c:if test="${ (loop.index) != paginateInfo.currentPage }">
				<a
					href="<c:url value="/search/key=${ keySearch }/${ loop.index }"/>">${ loop.index }</a>
			</c:if>
		</c:forEach>
	</div> 
</body>